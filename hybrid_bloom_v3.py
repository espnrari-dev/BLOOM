...
def get_spy():
    ...
    try:
        ...
        closes=[c for c in j["indicators"]["quote"][0]["close"] if c]
        vols=[v for v in j["indicators"]["quote"][0]["volume"] if v]
        return closes, vols
    except: return None,None

closes,volumes=get_spy()
MARKET=closes is not None
if MARKET: N=len(closes)

try:
    seed=json.load(open(GARDEN/"seedling.json")); dew=json.load(open(GARDEN/"dew.json"))["dew"]; total=seed.get("total_steps",0)
except:
...
def market_signal(idx):
    ...
    i=idx%N
    start=max(0,i-20); w=closes[start:i+1]; l=len(w) or 1
    p=closes[i]; ma20=sum(w)/l
    mom=max(0,min(10,(p/ma20-0.95)*100))
    vol=volumes[i]; vma=sum(volumes[start:i+1])/l or 1
    liq=max(0,min(10,vol/vma*5))
    start200=max(0,i-200); ma200=sum(closes[start200:i+1])/max(1,i-start200+1)
    val=max(0,min(10,10-abs(p/ma200-1)*100))
    mean=sum(w)/l; var=sum((x-mean)**2 for x in w)/l if l>1 else 0.01
    volat=max(0,min(10,math.sqrt(var)/mean*200 if mean!=0 else 0))
...
for _ in range(400):
    if dew<=0:
        print(f"DEATH {total} dew {dew:.1f} gen {seed['gen']} risk {seed['petals']['risk']['mean']:.2f}")
        open(GARDEN/"lineage.jsonl","a").write(json.dumps({"id":seed["id"],"gen":seed["gen"],"lifespan":lifespan})+"\n")
    ...
    moral_sig,regime=moral_regime(total); sig={**moral_sig,**market_signal(total)}
    prob=sigmoid(vigor(sig,seed)); will=prob>0.5
    true_noble=(sig["need"]+sig["oath"])>(sig["power"]+sig["risk"]*1.2)
    if regime==2: true_noble=False
    market_truth=(sig["momentum"]+sig["value"])>sig["volatility"]*1.3
    did=(true_noble and market_truth) if random.random()<0.78 else not (true_noble and market_truth)
    hit=will==did; hits.append(hit); lifespan+=1
    payoff=((-1.0 if hit else -2.0)+(2.5 if regime in [1,3] else 0.3)) if will else ((0.8 if not hit else 0.3)+(-1.5 if regime in [1,3] else 0))
    dew=dew-1.0+payoff
    open(GARDEN/"petals.jsonl","a").write(json.dumps({"total":total,"gen":seed["gen"],"regime":regime,"hit":hit,"dew":round(dew,1)})+"\n")
    err=(1.0 if did else 0.0)-prob; lr=0.09 if regime==3 else 0.06
    for k in sig: seed["petals"][k]["mean"]=seed["petals"][k]["mean"]*0.99+lr*err*sig[k]
    seed["root"]["mean"]=seed["root"]["mean"]*0.99+lr*err
    total+=1
    if total%25==0: print(f" step {total} regime {regime} dew {dew:.1f} acc25 {sum(hits[-25:])/25:.2f} risk {seed['petals']['risk']['mean']:.2f} need {seed['petals']['need']['mean']:.2f}")
...
all_p=[json.loads(l) for l in open(GARDEN/"petals.jsonl")]; by=defaultdict(list)
for p in all_p: by[p["regime"]].append(p["hit"])
print(f"\nV3 total {len(all_p)} gen {seed['gen']} dew {dew:.1f} market={MARKET}")
for r in sorted(by):
    label={0:"plenty",1:"scarcity",2:"temptation",3:"crisis"}[r]
    print(f" {label}: {sum(by[r])/len(by[r]):.2f} n={len(by[r])}")
print(f" soul need {seed['petals']['need']['mean']:.3f} risk {seed['petals']['risk']['mean']:.3f} oath {seed['petals']['oath']['mean']:.3f} power {seed['petals']['power']['mean']:.3f}")
