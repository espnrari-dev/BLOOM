...
print(f"PURE BEST vocab {vs} data {len(txt)}")

hidden=256; block=64; batch=32
Wxh=np.random.randn(vs,hidden)*0.08
Whh=np.random.randn(hidden,hidden)*0.08/np.sqrt(hidden)
Why=np.random.randn(hidden,vs)*0.08
...
def forward(xs,h0):
    T,B=xs.shape
    h=np.zeros((T+1,B,hidden)); h[0]=h0
...
def loss_grad(logits,targets,h,xs):
    T,B,_=logits.shape
    mx=np.max(logits,axis=-1,keepdims=True); e=np.exp(logits-mx); p=e/np.sum(e,axis=-1,keepdims=True)
...
for it in range(1,5001):
    ...
    for par,m,v,d in [(Wxh,mWxh,vWxh,dWxh),(Whh,mWhh,vWhh,dWhh),(Why,mWhy,vWhy,dWhy),(bh,mbh,vbh,dbh),(by,mby,vby,dby)]:
        ...
        par-=lr*(m/(1-0.9**it))/(np.sqrt(v/(1-0.999**it))+1e-8)
    if it%100==0:
        cur=np.array([[stoi['\n']]]); hs=np.zeros((1,hidden)); out=""
        for _ in range(300):
            lg,hf=forward(cur,hs); hs=hf[-1]
            prob=np.exp(lg[-1]-np.max(lg[-1])); prob/=prob.sum()
            nxt=np.random.choice(vs,p=prob[0]); out+=itos[nxt]; cur=np.array([[nxt]])
        print(f"{it} loss {loss:.3f} best {best:.3f}\n{out[:300]}\n---")
        best=min(best,loss)
